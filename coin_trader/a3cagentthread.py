...
class A3CAgentThread(threading.Thread):
    ...
    def run(self):
        ...
        for episode in range(self.num_episodes):
            ...
            # 10개 항목 이후부터 시작
            for i in range(10, len(preprocessed_df) - 1):
                # 행동 선택
            ...
            total_current_value += self.agent.current_value
            total_buy_count += buy_count
            total_sell_count += sell_count
            total_hold_count += hold_count
            self.agent.save("combined_models/ppo_model.h5")
...
if __name__ == "__main__":
    ...
    a3c_thread = A3CAgentThread(agent, num_episodes=100, interval=3, count=2000)
    a3c_thread.start()
